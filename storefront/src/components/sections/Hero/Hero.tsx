...
export const Hero = ({ image, heading, paragraph, buttons }: HeroProps) => {
  return (
    <section className="relative w-full container mt-5 overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-20 pointer-events-none">
        ...
        <div className="absolute bottom-20 left-10 w-80 h-80 bg-gradient-to-r from-purple-300/30 to-pink-300/30 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }}></div>
      </div>

      <div className="relative flex flex-col lg:flex-row text-primary gap-8">
        
        {/* Image Section with Glassmorphism Frame */}
            ...
            <div className="h-[72px] lg:h-[144px] flex font-bold uppercase gap-4">
              {buttons.map(({ label, path }, index) => (
                ...
                >
                  ...
                  <div className="relative z-10 flex h-full p-6 justify-between items-end">
                    <span className="text-lg lg:text-xl">
                      <span className="group-hover:inline-flex hidden animate-pulse">#</span>
                      {label}
                    </span>

                    <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 group-hover:scale-110 group-hover:rotate-12 ${
                      index === 0 
                        ? 'bg-white/20' 
                      ...
                      <svg 
                        ...
                        className={index === 0 ? 'text-white' : 'text-white'}
                      >
